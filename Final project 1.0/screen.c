...
static void WriteChar_Fonts(uint8_t *buf, int16_t anchor_x, int16_t anchor_y, uint8_t ch, const unsigned char *font){
    // if (reversed[0] == 0) 
    //     FillReversedCache();
    ...
    if (anchor_x > SSD1306_WIDTH - font_width || anchor_y > SSD1306_HEIGHT - font_height)
        return;
        ...
        for (uint8_t x = 0; x < font_width; x++) {
            for (uint8_t y = 0; y < font_height; y++) {
                if (font[seek] >> b_seek & 0b00000001) {
                    SetPixel(buf, x + anchor_x, y + anchor_y, true);
                }
                b_seek++;
                if (b_seek == 8) {
                    b_seek = 0;
...
static void WriteString_Fonts(uint8_t *buf, int16_t x, int16_t y, char *str, const unsigned char *font){
    // Cull out any string off the screen
    if (x > SSD1306_WIDTH - font[0] || y > SSD1306_HEIGHT - font[1])
        return;

    while (*str) {
        WriteChar_Fonts(buf, x, y, *str++, font);
        x+=font[0];
    }
}
...
void set_and_show_menu( char titolo[],  char opzione1[],  char selezionato[],  char opzione2[]){
    char *text[] = {
        titolo,
        opzione1,
        selezionato,
...
void write_menu_on_screen(char TITOLO[],char sottotitolo1[],char sottotitolo2[],char sottotitolo3[]){
    ...
    WriteString_Fonts(buf, len, 2, TITOLO, font_12x16);
    WriteString_Fonts(buf, 25, 25, sottotitolo1, font_5x8);
    WriteString_Fonts(buf, 0, 38, "=>", font_8x8);
    WriteString_Fonts(buf, 25, 38, sottotitolo2, font_8x8);
    WriteString_Fonts(buf, 25, 50, sottotitolo3, font_5x8);
