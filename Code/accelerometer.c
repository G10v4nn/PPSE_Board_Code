...
void mma8451_set_state(uint8_t state) {
    bufacc[0] = REG_CTRL_REG1;
    bufacc[1] = state; // Set RST bit to 1
    i2c_write_blocking(i2c_default, ADDRESS, bufacc, 2, false);
}
#endif
...
#warning i2c/mma8451_i2c example requires a board with I2C pins
    ...
    bufacc[0] = REG_DATA_CFG;
    bufacc[1] = range_config;
    i2c_write_blocking(i2c_default, ADDRESS, bufacc, 2, false);
...
void get_accelerometer(struct Data_storage *Data){
    ...
    i2c_read_blocking(i2c_default, ADDRESS, bufacc, 2, false);
    x_acceleration = mma8451_convert_accel(bufacc[0] << 6 | bufacc[1] >> 2);
    Data->x_acceleration = x_acceleration;

    i2c_write_blocking(i2c_default, ADDRESS, &REG_Y_MSB, 1, true);
    i2c_read_blocking(i2c_default, ADDRESS, bufacc, 2, false);
    y_acceleration = mma8451_convert_accel(bufacc[0] << 6 | bufacc[1] >> 2);
    Data->y_acceleration = y_acceleration; 

    i2c_write_blocking(i2c_default, ADDRESS, &REG_Z_MSB, 1, true);
    i2c_read_blocking(i2c_default, ADDRESS, bufacc, 2, false);
    z_acceleration = mma8451_convert_accel(bufacc[0] << 6 | bufacc[1] >> 2);
